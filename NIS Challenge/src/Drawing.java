...
public class Drawing {
	...
	// The drawing is the action that drives the simulation therefore  it is treated as the main method
	public static void main(String[] args) {
		...
		while(!lm.checkIfEmpty())
		{
			...
			int[] winningNumbers = {lm.getWinner(0).getTicketValue(), lm.getWinner(1).getTicketValue(), lm.getWinner(2).getTicketValue()};
		...
		System.out.println("Winning numbers are: " + String.format("%03d", lm.getWinner(0).getTicketValue()) + "  " + String.format("%04d", lm.getWinner(1).getTicketValue())+ "  " + String.format("%04d", lm.getWinner(2).getTicketValue()));
	}
